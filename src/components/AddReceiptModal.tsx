...
import { createReceipt, uploadReceiptImage, syncTripSpent } from '../services/receiptsService';
...
export default function AddReceiptModal({ visible, tripId, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    if (visible && tripId) {
      getStops(tripId).then(setStops).catch(console.error);
    }
  }, [visible, tripId]);
  ...
  async function pickImage(fromCamera: boolean) {
    ...
    if (permissionResult.status !== 'granted') {
      Alert.alert(
        'Permission needed',
        'Photo permissions are required to add receipt images.',
      );
      return;
    }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const tempId = `receipt_${Date.now()}`;
      let imageUrl: string | null = null;

      if (imageUri) {
        imageUrl = await uploadReceiptImage(imageUri, tempId);
      }
