...
type Props = {
  ...
  onClose: () => void;
  onSave: (id: string, updates: Partial<ReceiptInsert>) => Promise<unknown>;
};

export function EditReceiptModal({ visible, receipt, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    ...
    setCategory(normalizeReceiptCategory(receipt.category));
  ...
  }, [receipt, merchant, amount, category, date, notes, onSave, onClose]);
